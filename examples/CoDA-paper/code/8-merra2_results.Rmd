...
if (!file.exists(fp_p_merra_fi)) {
  ...
  p_merra_fi <-
    ...
    geom_line(
      mapping = aes(x = t_adj, y = fi, group = esn), 
      alpha = 0.5,
      color = "grey60"
    ) +
    geom_line(
...
Comparing number of blocks (averaged over ESNs):
...
Comparing hyperparameter values (averaged over ESNs):
...
if (!file.exists(fp_p_merra_fi_rmses)) {
  ...
  fi_rmses <- 
    ...
    filter(nh == 1000, reg_par == 5) |>
    select(esn, feat_imp, nblocks, variable, t_adj,
           rmses_obs, rmses_adj) |>
  ...
  p_merra_fi_rmses <-
    ...
    geom_line(aes(y = rmse, group = factor(esn):factor(rmse_type)), 
