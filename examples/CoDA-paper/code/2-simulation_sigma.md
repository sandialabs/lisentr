...
Implement simulation study for comparing effects of varying the sigmas:

``` r
fp_sim_sigma =  "../output/simulation_res_sigma.csv"
if (!file.exists(fp_sim_sigma)) {
  cl <- makeCluster(8)
  ...
  results <-
    foreach(
      ...
      for (j in 1:nsims) {
        ...
        ta6_train_mats <-
          set_names(c("val", "x1", "x2")) |> map(.f = prep_train_mat_ta6)
  ...
  # Save results
  write.csv(resultsdf, file = fp_sim_sigma)
}
```
