---
title: 'MERRA2: Hyperparameter Tuning'
...
**Overview**: This script implements a hyperparameter search and compares RMSE variability across different ensembles sizes for ESNs trained using data from 1980 through 1995.
...
Specify training/testing inputs/outputs (AOD and stratospheric temperature are used to forecast stratospheric temperature):
...
Extract the times associated with the training/testing data (note that the times for `x` and `y` are the same):
...
These visualizations lead me to select 5 PCs over 10 PCs and to determine that unimportant hyper parameters are W_width, U_pi, and W_pi:

```{r echo = FALSE}
# Specify figure file path
fp_p_hp_all = "../figs/merra2_hyper_param_all.png"

# Create/load plot
if (!file.exists(fp_p_hp_all)) {
  
  # Create plot
  plot_test_v_train <- function(param, param_tex) {
    ncol = hs |> pull({{param}}) |> unique() |> length()
    ggplot(hs, aes(x = mRMSE_tr, y = mRMSE_te, color = .data[[param]])) +
      geom_point(alpha = 0.25, size = 0.25) +
      viridis::scale_color_viridis(
        option = "plasma", direction = -1, discrete = T
      ) +
  ...
  param_plots = map2(.x = params, .y = params_tex, .f = plot_test_v_train)
  p_hp_all = cowplot::plot_grid(plotlist = param_plots)
  
  # Save plot
  ggsave(
    filename = fp_p_hp_all,
    plot = p_hp_all,
    dpi = 500,
    width = 15,
    height = 11
...
# Load plot
knitr::include_graphics(fp_p_hp_all)
```

These visualizations tell me that the parameters most associated with a low testing RMSE are nh and U_width. These are followed by nu and reg_par. It is still unclear whether add_quad is helpful or not.

```{r echo = FALSE}
# Specify figure file path
fp_p_hp_sub = "../figs/merra2_hyper_param_sub.png"

# Create/load plot
if (!file.exists(fp_p_hp_sub)) {
  
  # Create plot
  plot_test_v_train_sub <- function(param, param_tex) {
    hs |>
      filter(npcs == 5, mRMSE_te < 1) |>
      ggplot(aes(x = mRMSE_tr, y = mRMSE_te, color = .data[[param]])) +
  ...
  param_plots_sub <-
    map2(
      .x = params[c(2,3,5,7,8,9)], 
      .y = params_tex[c(2,3,5,7,8,9)], 
      .f = plot_test_v_train_sub
    )
  p_hp_sub = cowplot::plot_grid(plotlist = param_plots_sub, ncol = 2)
  
  # Save plot
  ggsave(
    filename = fp_p_hp_sub,
    plot = p_hp_sub,
    dpi = 500,
    width = 15,
    height = 11
...
# Load plot
knitr::include_graphics(fp_p_hp_sub)
```

U_with = 0.1 performs the best across all values of nh and which value of reg_par does the best is dependent on U_width and nh:

```{r echo = FALSE}
# Specify figure file path
fp_p_hp_uwidth = "../figs/merra2_hyper_param_uwidth.png"

# Create/load plot
if (!file.exists(fp_p_hp_uwidth)) {
  
  # Function for labeling facets
  nh_appender <- function(string) TeX(paste("$n_h = $", string))
   
  # Create plot
  p_hp_uwidth <-
    hs |>
    filter(npcs == 5) |>
    ggplot(aes(x = U_width, y = mRMSE_te)) +
    geom_violin() +
    geom_jitter(aes(color = reg_par),
                size = 0.5,
                alpha = 0.5,
                width = 0.1) +
    facet_grid(. ~ nh, labeller = as_labeller(nh_appender,
...
# Load plot
knitr::include_graphics(fp_p_hp_uwidth)
```

Best reg_par does not appear to be dependent on nu and in general, a lower nu performs better within each value of nh (going forward subset nu = 0.2):

```{r echo = FALSE}
# Specify figure file path
fp_p_hp_nu = "../figs/merra2_hyper_param_nu.png"

# Create/load plot
if (!file.exists(fp_p_hp_nu)) {
  ...
  p_hp_nu <-
    hs |>
    filter(npcs == 5, U_width == 0.1) |>
    ggplot(aes(x = nu, y = mRMSE_te)) +
    geom_violin() +
    geom_jitter(aes(color = reg_par), alpha = 0.5, width = 0.15) +
    facet_grid(. ~ nh, labeller = as_labeller(nh_appender,
                                              default = label_parsed)) +
    viridis::scale_color_viridis(option = "plasma",
                                 direction = 1,
...
# Load plot
knitr::include_graphics(fp_p_hp_nu)
```

These plots highlight the best performing model (nh of 1000, reg_par of 5), but there are other cases that perform close including nh of 2000 and reg_par of 50. Even nh of 50 with reg_par of 0.1 is within a range that might be considered not practically different. Note that this plot also suggests that add_quad has more effect when reg_par is small and minimal impact with reg_par increases:

```{r echo = FALSE}
# Specify figure file path
fp_p_hp_nh_quad = "../figs/merra2_hyper_param_nh_quad.png"

# Create/load plot
if (!file.exists(fp_p_hp_nh_quad)) {
  
  # Function for labeling facets
  lambda_appender <- function(string) TeX(paste("$\\lambda_r = $", string))
  
  # Create plot
  p_hp_nh_quad <-
    hs |>
    filter(npcs == 5, U_width == 0.1, nu == 0.2) |>
    ggplot(aes(x = nh, y = mRMSE_te)) +
    geom_violin() +
    geom_jitter(aes(color = add_quad), alpha = 0.5, width = 0.15) +
    facet_grid(
      . ~ reg_par,
      labeller = as_labeller(lambda_appender, default = label_parsed)
    ) +
    viridis::scale_color_viridis(option = "plasma",
                                 direction = 1,
                                 discrete = T) +
...
# Load plot
knitr::include_graphics(fp_p_hp_nh_quad)
```

This plot considers the effect of U_pi with the best performing other hyperparameters. We see that the value of U_pi associated with the lowest RMSE changes based on the value of reg_par:

```{r echo = FALSE}
# Specify figure file path
fp_p_hp_nh_upi = "../figs/merra2_hyper_param_nh_upi.png"

# Create/load plot
if (!file.exists(fp_p_hp_nh_upi)) {
  ...
  p_hp_nh_upi <-
    ...
    ggplot(aes(x = nh, y = mRMSE_te)) +
    geom_violin() +
    geom_jitter(aes(color = U_pi), alpha = 0.5, width = 0.15) +
    facet_grid(
      . ~ reg_par,
      labeller = as_labeller(lambda_appender, default = label_parsed)
    ) +
...
# Load plot
knitr::include_graphics(fp_p_hp_nh_upi)
```
...
Function for training an EESN and returning the RMSE. For a specified number of ESNs per ensemble and seed:

1. Train ensemble of ESNs
2. Get predictions from each ensemble
3. For each spatial location and time, average prediction across ensembles
...
Train ESN ensembles with different number of base models and extract weighted RMSEs:
...
```{r echo = FALSE}
# Specify figure file path
fp_p_nens = "../figs/merra2_nens_comparison.png"

# Create/load plot
if (!file.exists(fp_p_nens)) {
  p_nens <-
    bind_rows(
      nens_sims_eesn1 |> mutate(params = 1),
      nens_sims_eesn2 |> mutate(params = 2),
      nens_sims_eesn3 |> mutate(params = 3)
    ) |>
    ggplot(aes(x = factor(n_ens), y = wgt_rmse)) +
    geom_violin() +
    facet_grid(params ~ ., scales = "free_y") +
    theme_bw() +
    labs(x = "Number of ensembles", y = "Ensemble RMSE")
...
# Load plot
knitr::include_graphics(fp_p_nens)
```
