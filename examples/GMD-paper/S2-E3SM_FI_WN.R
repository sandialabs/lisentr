# This script trains an ESN and FI to the E3SM fullvar 8 year run data with white noise added as covariate
...
out <- foreach(i=1:5,.packages=c('listenr','dplyr','lubridate','tidyr','tidync','stringr')) %dopar% {
  ...
  # add white noise
  e3sm_train$noise <- rnorm(nrow(e3sm_train))
  ...
  eofs_WN = compute_eofs(Ztrain = train_mat_WN, n_eofs = n_eofs)
  
  
  
  # Model matrices
  x_T050 = cbind(eofs_AEROD_v$train, eofs_FLNT$train, eofs_WN$train, eofs_T050$train)
  ...
  x_TREFHT = cbind(eofs_AEROD_v$train, eofs_FSDSC$train, eofs_WN$train, eofs_TREFHT$train)
  ...
  zfi_res_T050 <-
    ...
    mutate(
      feat_imp = "ZFI",
      variable = ifelse(vars_adj == paste(1:n_eofs,collapse=","), "AEROD_v",ifelse(vars_adj == paste((n_eofs+1):(2*n_eofs),collapse=','), "FLNT",ifelse(vars_adj==paste((2*n_eofs+1):(3*n_eofs),collapse=','),"WN","T050"))), 
  ...
  zfi_res_TREFHT <-
    ...
    mutate(
      feat_imp = "ZFI",
      variable = ifelse(vars_adj == paste(1:n_eofs,collapse=","), "AEROD_v",ifelse(vars_adj == paste((n_eofs+1):(2*n_eofs),collapse=','), "FSDSC",ifelse(vars_adj==paste((2*n_eofs+1):(3*n_eofs),collapse=','),"WN","TREFHT"))), 
      t_forecasted = as_date(t_forecasted),
      t_adj = as_date(t_adj)
    ) %>%
    mutate(month_diff = interval(floor_date(as_date(t_adj),'month'),floor_date(as_date(t_forecasted),'month')) %/% months(1)) %>%
...
write.csv(fi,file=paste0("../output/gmd_paper/104b-e3sm_fullvar_WN.csv"),row.names=FALSE)
