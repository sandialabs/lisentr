# This script trains an ESN and FI to the E3SM fullvar 8 year run data with no AOD
...
out <- foreach(i=1:5,.packages=c('listenr','dplyr','lubridate','tidyr','tidync','stringr')) %dopar% {
  ...
  b1=e3sm %>% group_by(lon,lat) %>% summarize(n2=sum(is.na(FSDSC)),n=sum(is.na(FSDSC_stndzd)),n3=n2+n)
  ...
  # Model matrices
  x_T050 = cbind(eofs_FLNT$train, eofs_T050$train)
  ...
  x_TREFHT = cbind(eofs_FSDSC$train, eofs_TREFHT$train)
  ...
  zfi_res_T050 <-
    ...
    mutate(
      feat_imp = "ZFI",
      variable = ifelse(vars_adj == paste(1:n_eofs,collapse=","),"FLNT",ifelse(vars_adj==paste((n_eofs+1):(2*n_eofs),collapse=','),"T050","TREFHT")), 
  ...
  zfi_res_TREFHT <-
    ...
    mutate(
      feat_imp = "ZFI",
      variable = ifelse(vars_adj == paste(1:n_eofs,collapse=","), "FSDSC",ifelse(vars_adj==paste((n_eofs+1):(2*n_eofs),collapse=','),"TREFHT","TREFHT")), 
      t_forecasted = as_date(t_forecasted),
      t_adj = as_date(t_adj)
    ) %>%
    mutate(month_diff = interval(floor_date(as_date(t_adj),'month'),floor_date(as_date(t_forecasted),'month')) %/% months(1)) %>%
...
write.csv(fi,file=paste0("../output/gmd_paper/104c-e3sm_fullvar.csv"),row.names=FALSE)
